...
use luaenv::{
    env::LuaEnv,
    lua::{Lua, LuaFunction, LuaTable, LuaValue},
};
use mlua::LuaString;
...
use crate::core::{FinishReason::ToolCall, Tool, ToolParameter};
...
impl From<(&Lua, ChatOptions)> for WLuaTable {
    fn from(value: (&Lua, ChatOptions)) -> Self {
        ...
        chat_options_table.set("provider", LuaValue::Nil).unwrap();
        chat_options_table
...
struct LuaFuncTool((Tool, LuaFunction));

struct Tools(Vec<LuaFuncTool>);
