...
pub fn chat(
    ...
    let tool_definitions: Vec<JsonValue> = tools.iter().map(|t| tool_to_json_value(t)).collect();
                ...
                if content_part == "[DONE]" {
                    ...
                    if let Ok(c) = serde_json::from_str::<OpenaiChunk>(content_part) {
                        ...
                        if finish_reason.is_none() {
                            chunk_receiver(MessageChunk::Chunk {
                                ...
                                reasoning_content: if let Some(rc) = &first_choice.delta.reasoning
                                    && rc.len() > 0
                                {
                                    Some(rc.to_string())
                                } else {
                                ...
                                tool_calls: if let Some(tc_list) = &first_choice.delta.tool_calls {
                                    Some(
                                        tc_list
                                            .iter()
                                            .map(|tc| ToolCall {
